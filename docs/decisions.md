...
Last updated: 2026-09-20, after adding save/restore, printing and the shirt mockup.
...
| **Two independent spacing dials, both measured out from the glyph** | `box = glyph + 2 * cellPadding` and `pitch = box + letterSpacing`. Cell padding is the room around a letter inside its own cell; letter spacing is the gap between cells. Deriving the box rather than fixing it is what keeps one dial from moving the other. The SVG is sized in real pixels and capped at the panel width, since with a viewBox alone the grid would just rescale and the sliders would appear to do nothing.                                |
| **Drafts save to `localStorage`, not a cookie**                     | Asked for as "a cookie", implemented as localStorage: cookies cap at about 4KB, which a long word list plus carved-cell overrides would silently truncate, and they ride on every request for no benefit on a static site. Saves on every change rather than behind a button, because an author who closes the tab did not decide to discard their work. The payload is versioned and every field is validated on load, since anything in storage is untrusted. |
| **Printing outputs the puzzle, not the page**                       | Done in CSS by hiding the app chrome rather than by building a second render, so the printed grid cannot drift from the previewed one. Letters are forced to black on white: the screen theme is light-on-dark and would print as a block of toner. The word list is a print-only element, because the on-screen one is a textarea and would print as a scrolled box.                                                                                           |
| **Answers never print**                                             | Independent of the on-screen highlight. Nobody wants the solution on the shirt, and a deliberate printed answer key is still open question 3.                                                                                                                                                                                                                                                                                                                   |
| **The shirt mockup reuses the preview's grid renderer**             | `GridCells` is shared and scaled into the chest area by a nested `<svg>`. A mockup that redrew the grid its own way would drift from the thing being approved. Body types are measurements, not hand-drawn paths, so a fourth is a data change. Ink is bound to the fabric rather than chosen separately, so the author cannot preview something unprintable.                                                                                                   |
...
2. **What does the t-shirt output need physically?** Grid plus word list is now a print
   toggle, so that half is answered. What remains is the physical side: dimensions, and
   whether anything needs to be print-shop-specific (bleed, colour mode, minimum line
   weight). Unanswered since before the pivot, and it still blocks the SVG export.

3. **Should a printed answer key exist at all?** On screen it is settled: a "Highlight
   answers" button beside Verify, for helping a solver who is stuck. Printing currently
   never includes it, which is the safe default. Whether there should be a deliberate
   "print the answer key" output, as a second page or a separate sheet, is undecided.

4. **Do shapes beyond the six presets matter?** SVG outline import and a text stencil (type
