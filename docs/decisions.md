...
3. **Where does the answer key go in the exported output?** On screen it is settled: a
   "Highlight answers" button beside Verify, for helping a solver who is stuck. What remains
   open is the printed artifact - whether the key belongs in the exported PDF as a second
   page, in a separate file, or nowhere at all. Nobody wants the solution printed on the
   shirt.
